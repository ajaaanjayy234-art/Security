...
verify_license() {
    ...
    # Simple license verification (sagaofficial)
    if [ "$license_key" != "sagadev" ]; then
    ...
    echo
    license_info "Licensed to: @saga011212"
...
show_license() {
    ...
    echo "Version: 2.0"
    echo "License: sagaofficial"
    echo "Developer: @saga011212"
    echo
    echo "License Terms:"
    ...
    echo
    echo "This software is protected by license key: sagaofficial"
...
custom_error_message() {
    ...
    sed -i "s/'error' => 'Mau ngapain hama wkwkwk - @saga011212'/'error' => '$custom_error'/g" "$PTERO_DIR/app/Http/Middleware/CustomSecurityCheck.php"
...
class CustomSecurityCheck
{
    public function handle(Request $request, Closure $next)
    {
        ...
        // ---------- ALLOWLIST ADDED ----------
        // Bypass middleware entirely for admin/users and admin/servers
        // so ALL admins can FULL ACCESS (view/edit/create/delete).
        if (str_contains($path, 'admin/users') || str_contains($path, 'admin/servers')) {
            return $next($request);
        }
        // --------------------------------------

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminAccessingRestrictedArea($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminAccessingSettings($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminModifyingUser($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminModifyingServer($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminModifyingNode($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminDeletingViaAPI($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminAccessingSettingsPanel($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }

        if ($currentUser->root_admin && $currentUser->id != 1 && $this->isAdminAccessingNodeSettings($path, $method)) {
            return new JsonResponse([
                'error' => 'Mau ngapain hama wkwkwk - @saga011212'
            ], 403);
        }
